...
>
> **A second, independent finding — `--usage-file` and `--resume` are not available on one invocation path.** Confirmed by reading `hermes_cli/oneshot.py::run_oneshot`'s actual signature (`model, provider, toolsets, skills, usage_file` — no `resume` parameter, and none of its 3 call sites in `main.py` pass one; `-z --resume` is dead code even though argparse silently accepts the flag) and by a hard argparse error when passing `--usage-file` to `chat`. `runtime/hermes.py` resolves this along the same seam the state machine already uses — full turns (never resume) go through `-z`, continuation turns (`chat -q --resume`, confirmed live to genuinely recall prior turns) recover usage after the fact via `hermes sessions export - --format jsonl --session-id <id>`, which carries the same accounting fields. A further live-verified wrinkle: on `chat -q --resume`, the `session_id:` marker prints to **stderr**, not stdout, so there is no structural separator on stdout between a CLI banner (e.g. a security-scanner notice) and the real response — `runtime/hermes.py` filters known status-icon prefixes (`⚠ ↻ ⚡ → ✓`) for that path specifically; `-z`'s stdout needs no cleaning at all, matching its own docs ("nothing else on stdout").
